...
describe("createHeuristicSourceToTestMap", () => {
  test("should map source files to test files based on naming conventions", () => {
    ...
    const result = indexModule.createHeuristicSourceToTestMap(
      sourceFiles,
      testFiles,
    );
  ...
  test("should handle empty input arrays", () => {
    ...
    expect(
      indexModule.createHeuristicSourceToTestMap(["lib/app/module.ex"], []),
    ).toEqual({
  ...
  test("should handle project prefixes in file paths", () => {
    const sourceFiles = ["my_project/lib/app/module.ex"];

    const testFiles = ["my_project/test/app/module_test.exs"];
...
+const d = 4;
 const e = 5;`;
    ...
    expect(result).toEqual(["const b = 2;", "const d = 4;"]);
  });
...
describe("analyzeCodeForTests", () => {
  ...
  beforeEach(() => {
    ...
    mockAnthropic = {
      messages: {
        create: jest.fn().mockResolvedValue({
          content: [
            {
              text: '{"tests":[], "create_new_file": false, "test_file_path": "test/example_test.exs", "complete_test_file": "test content", "lowest_confidence": 3}',
            },
          ],
        }),
      },
    ...
    jest
      ...
      .mockImplementation(() => "existing test content");
  ...
  test("should call Anthropic API with correct parameters", async () => {
    ...
    const prompt =
      mockAnthropic.messages.create.mock.calls[0][0].messages[0].content;
  ...
  test("should handle different response formats", async () => {
    const testCases = [
      {
        response:
          '{"tests":[{"target":"sum/2","explanation":"Tests basic addition","confidence":4}], "create_new_file": true, "test_file_path": "test/new_test.exs", "complete_test_file": "content", "lowest_confidence": 4}',
        expected: {
          tests: [
            {
              target: "sum/2",
...
describe("run", () => {
  ...
  beforeEach(() => {
      ...
      rest: {
        pulls: {
          ...
          get: jest
            .fn()
            .mockResolvedValue({
              data: { head: { ref: "test-branch", sha: "test-sha" } },
            }),
        ...
        git: {
          getRef: jest
            .fn()
            .mockResolvedValue({ data: { object: { sha: "test-sha" } } }),
          getCommit: jest
            .fn()
            .mockResolvedValue({ data: { tree: { sha: "test-tree-sha" } } }),
          createBlob: jest
            .fn()
            .mockResolvedValue({ data: { sha: "test-blob-sha" } }),
          createTree: jest
            .fn()
            .mockResolvedValue({ data: { sha: "test-new-tree-sha" } }),
          createCommit: jest
            .fn()
            .mockResolvedValue({ data: { sha: "test-new-commit-sha" } }),
    ...
    rewiredModule.__set__(
      "generateRepoMap",
      jest.fn().mockReturnValue(["lib/app/accounts.ex"]),
    );
    rewiredModule.__set__(
    ...
    rewiredModule.__set__(
      "extractAddedLines",
      jest.fn().mockReturnValue(["def sum(a, b), do: a + b"]),
    );
    rewiredModule.__set__(
      "analyzeCodeForTests",
      jest.fn().mockResolvedValue({
    ...
    jest
      .spyOn(fs, "readFileSync")
      .mockImplementation(() => "mock file content");
  ...
  test("should process PR files and analyze code for tests", async () => {
    ...
    const mapSourceFilesToTestFiles = rewiredModule.__get__(
      "mapSourceFilesToTestFiles",
    );
  ...
  test("should implement pending test files when tests have high confidence", async () => {
    ...
    const implementPendingTestFiles = rewiredModule.__get__(
      "implementPendingTestFiles",
    );
